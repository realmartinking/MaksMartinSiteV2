...
List and Grid suspend pointer hover switching while scrolling and restore the project under the cursor after motion settles. Keyboard focus remains immediate. This avoids repeated video preview starts, blur/grayscale transitions, and React updates as content passes a stationary pointer. Grid project content is memoized. Near rows retain their compositor plane through the flat focus region, with a smaller offscreen working range. The row geometry, 62° tilt, 150px depth, 6px blur, and original media files are preserved.

The desktop emblem/name lockup is 7px left of the previous release (5px extra inset). The rotation stage remains uncropped. See [the scroll measurements](docs/scroll-polish-2026-09-16.md) for performance evidence and browser limitations.

### Video readiness correction

Every project video has a native-resolution WebP cover from its own 0.5-second frame (18 images, 898,044 bytes combined). `ProjectVideo` keeps this independent image above the native player until playback submits an actual frame through `requestVideoFrameCallback`. Metadata or a resolved `play()` alone does not remove the cover. Older browsers wait for `playing` and two animation frames. Media errors or emptied buffers restore the cover; a paused preload frame does not replace it. Video files, dimensions, frame rates and the original visual effects are unchanged.

One offscreen instance per source prepares metadata immediately after mounting and buffers within 1600px. Visible copies begin playback within 240px. Infinite duplicate instances do not each prebuffer the same source; ownership transfers when virtual cards recycle. Warming never calls `load()` to reset an active resource. Interrupted play is retried once; `canplay`, visibility restoration and a user interaction provide recovery without an unbounded retry loop. The shared player also drives desktop List previews and mobile inline expansion.

Validation: 29 motion/media tests, TypeScript, browser checks of all view types, and a temporary route with unavailable video URLs whose six cards kept their covers visible. That route was removed before release. Direct Safari rendering and physical-device touch performance have not been measured; browser testing used Chromium. The previous Intel measurements describe the prior release, not a new Safari FPS guarantee.

Covers were generated with FFmpeg: `ffmpeg -ss 0.5 -i input.mp4 -frames:v 1 -c:v libwebp -quality 90 -compression_level 6 output.webp`. The playback and poster approach follows [Apple’s Safari media guidance](https://developer.apple.com/documentation/webkit/delivering-video-content-for-safari) and [the video frame callback API](https://developer.mozilla.org/en-US/docs/Web/API/HTMLVideoElement/requestVideoFrameCallback).
